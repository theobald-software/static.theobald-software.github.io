...
(function() {
    ...
    // t.readyDeferred can/will be overridden by extended libs
    t.readyDeferred = t._readyDeferred = ecsDeferred.promise();

    //
    // Main enter point for external functions
    //

    // t.ready is overridden by extended libs
    t.ready = t._ready = function(f) {
...
define('theobald.util',[],function() {
    ...
    //
    // string formatters (useful for SAP messages and parameters)
    //
    ...
    util.ltrim = function(str, chars) {
        chars = chars || '\\s';
        return str.replace(new RegExp('^[' + chars + ']+', 'g'), empty);
    };
    ...
    util.padLeft = function(str, l, c) {
        // implicit conversion for e.g. numbers
        var strString = str + empty,
            len = l - strString.length + 1;
        return len > 0 ? Array(len).join(c || space) + strString : strString;
    };

    // padRight('abcd')     >>>     abcd000
    util.padRight = function(str, l, c) {
        var strString = str + empty;
        return strString + Array(l - strString.length + 1).join(c || space);
    };
...
define('theobald.ecs',['theobald.ajax', 'theobald.util'], function (_ajax, _util) {
    ...
    p.version = '5.7.0';
    p.timeStamp = 'Wed Apr 24 2019 18:42:28 GMT+0200 (Central European Summer Time)';
    ...
    p.localizableStrings = {
        ...
        de: {
            ...
            info: {
                ...
                credentialDeleted: "Benutzer wurde entfernt.",
    ...
    // ends underlying ajax request if any.
    p.abortRunningRequest = function (requestDeferred) {
        for (var deepDeferred = requestDeferred; deepDeferred && !deepDeferred.abort;) {
            deepDeferred = deepDeferred._deferred;
        }

        if (deepDeferred && deepDeferred.abort) {
            deepDeferred.abort();
        }
    }
    ...
    p.executeAjaxRequest = function (parameters) {
        ...
        var self = this,
            ...
            requiresInstanceAsUrlParameter = parameters._requiresInstanceAsUrlParameter,
        ...
        if (useSharepointSettings && t.sp && window.SP) {
            ...
            deferred._deferred.then(function (spSettings) {
                    ...
                    if (spSettings.instance) {
                        // some action (testproxy) accept only urlparameters
                        if (requiresInstanceAsUrlParameter) {
                            if (!urlParameters) {
                                urlParameters = {};
                            }
    ...
    p.executeEcsCommand = function (parameters, action) {
        ...
        var addCustomInstanseToParameters = function (asUrlParameter) {
                ...
                } else if (asUrlParameter) {
                    // action requires parameter as url, if instance is not set now, that could mean lazy init e.g. in sharepoint.
                    // set flag, so lazy setter recognizes this
                    updatedParameters._requiresInstanceAsUrlParameter = true;
                }
    ...
    p.getFullMessage = function (error, html) {
        return t.ecs.getLongErrorMessage(error, html);
    };
    ...
    p.callSoapService = p.wsdSoapQuery = function (options) {
        if (!options) {
            this.enull('options');
        }
        var self = this,
            ...
            doneHandler = function () {
                //data, xhr
                self.log('Service OK');
                _ajaxDeferred.resolve.apply(_ajaxDeferred, arguments);
            },
            failHandler = function (jqXhr, textStatus, errorThrown) {
                self.error('Service ERROR: %s, %o', textStatus, errorThrown);
                _ajaxDeferred.reject.apply(_ajaxDeferred, arguments);
            },
            ajaxOptions = {
        ...
        // overrides
        if (options.ajax) {
            var oAjax = options.ajax;
            this._extend(ajaxOptions, oAjax);

            if (oAjax.done || oAjax.success) {
                _ajaxDeferred.done(oAjax.done || oAjax.success);
            }
            if (oAjax.fail || oAjax.error) {
                _ajaxDeferred.fail(oAjax.fail || oAjax.error);
            }
        }

        // force done/fail to process the deferred
        ajaxOptions.done = doneHandler;
        ajaxOptions.fail = failHandler;

        _ajaxDeferred.__xhr = self.ajax(ajaxOptions);

        if (options.done || options.success) {
            _ajaxDeferred.done(options.done || options.success);
        }
        if (options.fail || options.error) {
            //jqXhr, textStatus, errorThrown
            _ajaxDeferred.fail(options.fail || options.error);
        }
        if (options.always) {
            _ajaxDeferred.always(options.always);
        }

        return _ajaxDeferred;
    };
    ...
    //    controls: { inputId: "", selectId: "", outputId: "", descriptionId: "", buttonId: "" },
    //    tableSettings: { tableName: "KNA1" idField: "KUNNR", textField: "NAME1", language: "EN" },
    //    searchOptions: { useUppercaseValuesForQueries: true, removeLeadingZerosFromNumbers: false, xql: "", xqlFormat: "", strings: null, german: true}
    //    connection: { ecs: {core: true, coreApiKey: "xxx", url: ""}  }
    // })
    // ECS_nintexFormsAnyTableCombobox_dev + ECS_nintexForms_Material_Combobox_OnType_de_caseinsensitive
    p.initializeLiveCombobox = function (options) {
        var $ = $ || theobald.$ || options.$;

        if (!options) {
            console.error("ECSJS: options must be defined (table and ui-controls information)");
            return void(0);
        }

        if (!options.controls) {
            console.error("ECSJS: controls not defined. Please use initializeLiveCombobox({controls: {xx}})");
            return void(0);
        }

        if (!$) {
            console.error("ECSJS: Please set the jQuery instance for using LiveCombobox. (options: {$ = NWA$})");
            return void(0);
        }

        var _searchOptions = this._extendSkipEmptyStrings({}, {
            ...
            // description output
            tsInputDescription = $('#' + controls.descriptionId),
            // button for manual search
            tsButton = $('#' + controls.buttonId),
            //
            // literals
            stringsEN = {
                loading: 'Loading...',
            ...
            // SELECT LANGUAGE
            strings = this._extendSkipEmptyStrings({}, (_searchOptions.german && stringsDE) || stringsEN, strings),
            //
            ...
            // xql query.
            xqlString = _searchOptions.xql || tEcs.format(_searchOptions.xqlFormat || "SELECT TOP 10 {0}, {1} FROM {2} WHERE ({0} LIKE '%{{0}}' OR {1} LIKE '%{{0}}%') AND SPRAS = '{3}' ",
            ...
            queryFunction = function (val) {
                var effectiveValue = _searchOptions.useUppercaseValuesForQueries ? val.toUpperCase() : val,
                    tEcsOptions = {
                        data: tEcs.format(xqlString, effectiveValue)
                    };

                if (options.connection) {
                    tEcsOptions.connection = options.connection;
                }
                ...
                .done(function (data) {
                    ...
                    if (data.length > 0) {
                        ...
                        $.each(data, function (i, v) {
                            ...
                            var idFieldValue = _searchOptions.removeLeadingZerosFromNumbers ? tEcs.util.ltrim(v[tableSettings.idField], '0') : v[tableSettings.idField];

                            $option.text(tEcs.format(domOptionString, idFieldValue, v[tableSettings.textField]));
        ...
        tsSelect.on('change', function () {
            ...
            tsOutputId.val(selectedOption.attr('tsid'));
    ...
    p.initialize = function () {
        ...
        t.ecs._extend(t.ecs, {
            ...
            _activateTraces: function () {
                t.ecs.trace = t.ecs.info;
            }
        });
